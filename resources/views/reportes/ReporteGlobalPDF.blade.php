...
<div class="table-container">
    <div style="margin-top: -4%;">
        ...
        <h4 class="text-center" style="margin-bottom: 1%;">Fondos recaudados por Fundamamas
            @if ($detalles)
                @if (!empty($eventoId))
                    en el evento: {{ $detalles->first()->evento ?? null }}
                @else
        ...
        <tbody class="bg-white">
            @if ($detalles && count($detalles) > 0)
                ...
                <tr style="background-color: #59de4d31;">
                    ...
                    <td class="p-4 font-bold">{{ $totalMonto }}</td>
                </tr>
            @else
                <tr>
                    <td colspan="7" class="p-4 text-center text-red-500">No hay detalles disponibles.</td>
                </tr>
            @endif
        </tbody>
    </table>
